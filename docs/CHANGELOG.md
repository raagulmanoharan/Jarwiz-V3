...
|---|---|---|
| #149 | `agents` · `cards` | Apply "Let Jarwiz fix it" inline and glow what it changed — the comment's fix button used to prefill the composer with the suggestion and wait for you to press Enter. It now applies the fix straight to the card (an in-place refine that keeps the card's format) and spotlights exactly what moved: a doc card glows the block(s) the fix rewrote (a full rewrite glows the whole card), a table card glows the changed cells — a soft one-shot fade (~2.4s), theme-aware and reduced-motion safe. The spotlight rides a transient per-shape store (`ask/fixHighlight`), kept OUT of `shape.meta` so it never lands in the document, undo history, or a room sync; one Cmd+Z still restores the card. The in-place doc refine now also clears `meta.jzBlocks` before streaming, so a rich answer replaces the old body instead of appending to it. A card whose format can't refine in place still falls back to prefilling the composer. Verified (Playwright): clicking the fix leaves the composer empty, updates the card inline, and glows the changed block (feedback) |
| #145 | `cards` | Fix a click below a short answer card selecting it — a doc card draws at its content height (`.jz-doc-auto` hugs the text), but hit-testing uses the shape's `props.h`, which started at the 240px default and (under the old `growOnly` fit rule) could only ever grow. So a short answer left a tall, *invisible* band below the visible text that still registered as the card: clicking the empty canvas just under a card selected it. The doc card's height now tracks its content exactly (dropped `growOnly`), so the shape box equals what's drawn and a click below it lands on empty canvas. On an auto-height card the taller box was always invisible, so nothing visible is lost. Verified (Playwright): a one-line card settles to ~54px (was 240), a click 40px below it deselects, a click on it still selects (bug report) |
| #143 | `agents` | Answer from the board + broad knowledge by default; clarify only on genuine ambiguity — the triage gate was too eager, so "his highest grosser" on a single-director board would still ask *which* director. The gate now defaults **hard** to answering: it resolves a terse reference ("his", "their", "these") from the card content or obvious context and just answers, answers anything reasonable from general knowledge even with no source attached, and **never** asks about output format. It returns a clarifying question ONLY when a reference could point to two or more clearly *different* things on the board (e.g. two directors) and it honestly cannot choose — two cards about the same subject is not ambiguous. Verified live (sidecar): one Nolan card → answers directly; Nolan + Spielberg cards → asks "Whose highest grosser — Nolan or Spielberg?" (feedback) |
| #142 | `agents` · `canvas` | Redesign the clarifying-question block as a clean card above the composer — it now docks in the prompt-bar stack directly above the composer, as wide as it, and hides the prompt-shortcut chips while it's up (one focused block, not a floating bubble anchored off to the side of a card). Stripped the ✦ spark motif; the layout is just the question, tappable suggestions, then an input. Moved `ClarifyLayer` out of the canvas overlay and into the dock (feedback) |
| #141 | `agents` · `cards` | Ground an unselected ask on the board's content when the board is small — asking "why was Memento not included" with nothing selected used to hit a titles-only wall ("I don't have any source documents"), because unselected asks passed only card *titles* (2026-07-20, for cost). Now a board of ≤ 8 cards sends each card's title AND its (capped) content, so the model answers directly from what's on the canvas; a bigger board still falls back to titles-only. Adds a shared `blocksToText` so block-stream cards (whose body lives in `meta.jzBlocks`, not `props.text`) are finally readable for grounding. The clarify triage sees the content too, so it stops asking for a source that's already there. Verified live: the Memento question now answers from a doc + table on the board (feedback) |
| #140 | `agents` · `canvas` | Stop an unselected ask that needs clarification from silently doing nothing — when the server returns a `clarify` question, the client retires the pre-placed card and hands the question to `ClarifyLayer`, which anchors it under the source card(s). But an *unselected* ask has no source card, so the anchor was null and the bubble rendered nowhere: you'd type, a placeholder would flash, then it'd vanish with no question and no answer. The clarify bubble now docks above the composer when there's nothing to anchor to, so the question (with its options) always appears (bug report) |
| #139 | `cards` · `agents` | Label a streaming answer card with your question — every pre-placed answer shape (table, diagram, prototype, dashboard, map) already seeded its title from the prompt, but the doc card (the common case) alone started blank, so a slow or stalled answer read as an anonymous "Writing this in…". The doc card now takes the prompt as its title the instant you press Enter (marked auto, so the server's content-derived title cleanly replaces it once it streams) — a cold-start wait now shows *your question, forming* instead of a blank box (feedback) |
| #138 | `agents` · `onboarding` | Fix the demo card telling people to click a "key button" that no longer exists — BYOK was pulled from the UI on 2026-07-10 ("server-side but out of the UI"), but the keyless demo answer still read "add your Anthropic API key (key button, top right)", sending hosted visitors hunting for a control that isn't there. The message now says it's demo mode and points at the paths that actually exist (a pilot invite, or a self-hosted server key). Real answers on the hosted app come from a `?pilot=CODE` invite; BYOK works only when self-hosting with `ANTHROPIC_API_KEY` set (bug report) |
| #137 | `cards` · `canvas` | Fix the whole-app crash when double-clicking a card — the edit-to-focus feature vetoed tldraw's `editingShapeId` write (nulling it to open focus mode instead of inline editing), but tldraw 5.1's select tool has already committed to entering its `editing_shape` state by then, and that state's `onEnter` throws "Entered editing state without an editing shape" when the id is null — so double-clicking ANY card showed tldraw's "Something went wrong / Reset data" crash screen. `EditToFocus` now lets the write land (satisfying `onEnter`) and opens focus + exits editing on the next microtask instead — before React paints, so still no inline-editor flash. Verified live: double-clicking a doc/block/note card opens focus with no crash (bug report) |
| #136 | `agents` · `cards` | Stop a streaming answer card from vanishing when generation fails — an answer streams its body into `meta.jzBlocks`, but the error path (`surfaceError`) and the abort path both judged a card "empty" by `props.text` alone, which is always blank for a block card, so a mid-stream failure or cancel discarded the whole card (you'd watch it write, then it'd disappear). A shared `cardIsEmpty` now counts a card with even one streamed block as content: an empty failed draft is still swept (with the error banner + Retry), but a partial answer is KEPT so a late/transient failure never wipes out what you just watched appear. Verified live: a card that took content then errored stays on the board with the banner shown (bug report) |
| #135 | `agents` · `cards` | Make AI-generated cards visible to board grounding — a block-stream card keeps its title as the first heading block in `meta.jzBlocks` (which renders inline), not in `props.title`, so `getShapeTitle` returned empty and an ungrounded ask's `boardIndex` (plus search and source-labelling) silently skipped every generated card. `getShapeTitle` now falls back to that first heading, and the outside title tag learns to stay quiet when the title is only that inline heading (no duplicate label over the card). Verified live: an ungrounded ask now carries the generated card's heading in `boardIndex`, with no visual title duplication (QA finding) |
| #134 | `cards` | Give the card shapes a shared `CardShapeUtil` base class — all 13 card ShapeUtils hand-copied the identical `getGeometry` (a filled w×h rectangle) and `canResize` (always true); those move to one `CardShapeUtil<T>` base (`shapes/CardShapeUtil.ts`) and each util now `extends CardShapeUtil<XCardShape>`, keeping only what makes it that card (props, `component`, indicator path, per-card resize mins). Zero behaviour change — the two hoisted methods are byte-identical to the originals; typecheck + build green (consolidation, part 6) |
