...
- ONLY when the numbers are the kind a reader would want to VARY and re-run — a cost or pricing model, unit economics, a forecast, a budget, a scorecard with weights, an ROI/break-even calc — include ONE move that turns it into a LIVE model they can adjust (inputs / sliders they change, outputs recomputing). Phrase it as building a tool and NAME the metrics ("Build a break-even calculator you can adjust", "Model the unit economics across cities"), and set that move's "shape" to "prototype". Set "shape":"prototype" on AT MOST ONE move, and NEVER on a comparison move (comparisons stay tables, no shape). Do NOT offer it for a static reference table that just reports facts (a listing count, a spec sheet, a results readout) — there is nothing to adjust there; leave those to the normal drill-in moves.
